...
public class Inventory {
    ...
    public List<String> getBuildables() {
        ...
        if ((wood >= 1 || arrows >= 2) && (keys >= 1 || treasure > sunStone || sunStone >= 2) && sunStone >= 1) {
            result.add("sceptre");
        }
    ...
    public InventoryItem checkBuildCriteria(Player p, boolean remove, String type, EntityFactory factory) {
        ...
        } else if (wood.size() >= 2 && (treasure.size() >= 1 || keys.size() >= 1) && type.equals("shield")) {
            if (remove) {
                items.remove(wood.get(0));
        ...
        } else if ((wood.size() >= 1 || arrows.size() >= 2) && (keys.size() >= 1 || treasure.size() > sunStone.size()
                    || sunStone.size() >= 2) && sunStone.size() >= 1 && type.equals("sceptre")) {
            if (remove) {
                if (wood.size() >= 1) {
                    items.remove(wood.get(0));
                } else {
        ...
        } else if (sword.size() >= 1 && sunStone.size() >= 1 && type.equals("midnight_armour")) {
            if (remove) {
