...
public class BattleTest {
    ...
    @Test
    @Tag("11-17")
    @DisplayName("Test midnight armour buff in battle")
    public void testMidnightArmourBuff() {
        ...
        DungeonManiaController controller = new DungeonManiaController();
        String config = "c_battleTest_TestMidnightArmourBuff";
        DungeonResponse res = controller.newGame("d_battleTest_TestMidnightArmourBuff", config);
        ...
        // Build Midnight Armour successful
        ...
        // Battle happens on next tick
        res = controller.tick(Direction.RIGHT);
        List<BattleResponse> battles = res.getBattles();
        assertTrue(battles.size() == 1);

        // Without the armour buff, there would be 5 rounds in the battle with
        // the player losing as each round the player takes 1/10 damage and the
        // mercenary takes 3/5 damage (see above battle stat values) But with the
        // buff there would still be 5 rounds but since the player takes less damage
        // and inflicts (3 + 2)/5 = 1 damage the mercenary loses.
        List<RoundResponse> rounds = battles.get(0).getRounds();
        assertTrue(rounds.size() == 5);

        List<EntityResponse> entitiesPostBattle = res.getEntities();
        assertEquals(1, TestUtils.countEntityOfType(entitiesPostBattle, "player"));
        assertEquals(0, TestUtils.countEntityOfType(entitiesPostBattle, "mercenary"));

        // The midnight armour still remains in the player's inventory and its buffs will continue
        // to apply in every battle
        assertEquals(1, TestUtils.getInventory(res, "midnight_armour").size());
    }
