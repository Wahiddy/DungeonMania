...
class MenuScene extends Scene {
  ...
  create() {
    ...
    // Newly added random dungeon button
    /////////////////////////////////////////////////
      ...
      async () => {
        ...
        const result = await Swal.fire({
          ...
          preConfirm: async () => {
            ...
            let results = [
              parseInt((document.getElementById("xStart") as HTMLInputElement).value),
              parseInt((document.getElementById("yStart") as HTMLInputElement).value),
              parseInt((document.getElementById("xEnd") as HTMLInputElement).value),
              parseInt((document.getElementById("yEnd") as HTMLInputElement).value),
            ];
            let configFile = (document.getElementById("config") as HTMLInputElement).value;
            if (await inputValidator(results)) {
              dungeon = await API.generateGame(results[0], results[1], results[2], results[3], configFile);
              return !!dungeon;
            }
          },
        });
    ...
    this.tweens.add({
      delay: 3500,
      duration: 1000,
      targets: [this.newGame, this.credits, this.exitGame, this.generateGame],
      x: 10,
      ease: "Bounce",
    });
